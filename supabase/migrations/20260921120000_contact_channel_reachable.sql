...
-- ---------------------------------------------------------------------------
-- account_contact_update: a changed email address clears the bounce state.
--
-- `delivery_state` is a system field — contract §2.3, never a user edit — and
-- it stays one. This is not the user overruling the mail provider's verdict:
-- it is the address that verdict was about no longer being on the row, so the
-- new one starts where every address starts, 'unverified'.
--
-- Unreachable until now, because nothing could change a contact's email. With
-- `Edit contact` on the card it is the first thing anyone will do to a bouncing
-- contact — and without this the account stays on "Can't chase — email
-- bouncing" for an address that no longer exists, which is precisely the state
-- the bounce strip sends them here to clear.
-- ---------------------------------------------------------------------------
...
begin
  ...
  v_email := case when p_body ? 'email'
                  then nullif(btrim(p_body ->> 'email'), '') else c.email end;
  ...
  update public.contacts set
    ...
    delivery_state   = case when v_email is distinct from c.email
                            then 'unverified'::public.delivery_state else delivery_state end,
    last_bounced_at  = case when v_email is distinct from c.email
                            then null else last_bounced_at end
  where id = c.id;
